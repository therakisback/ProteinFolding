...
# This is the file I used for the majority of my testing, in an attempt to squeeze out a bit more performance. 
# There may be small differences between this and the .ipynb file that I have not noticed, as I did not always keep perfect parity between the two
...
num_data = -1000 #grabs all but the last 1000 residues, as those will be saved for testing
train_data = vhseDataset(num_data = -1000)
test_data = vhseDataset(num_data = 1000, test = True)

train_data_pt = ptDataset(num_data = -1000)
test_data_pt = ptDataset(num_data = 1000, test = True)

# Experimentally testing time complexity - Doing steps of 4000 and is allows the testing time to be reasonable

# This will be used to determine the time of extracting the data from file, so it can be removed from the future runs (to show encoding time instead)
# start_time = time.time()
# vhseDataset(num_data = 1, test = True)
# print(f"Elapsed time: {time.time() - start_time}")

# This is not a perfect way to test time complexity, but the time to encode each can be deduced. In my gathering of data I used this method.
start_time = time.time()
vhseDataset(num_data = 1000, test = True)
print(f"Elapsed time: {time.time() - start_time}")
...
class CNN(nn.Module):
    def __init__(self, feat, channels):
        ...
        self.hidden = nn.Sequential(
            ...
            # TODO These are me experimenting with deeper CNNs to see if performance is altered. Marked TODO so I hopefully remember to remove them if I do not use it
            nn.Conv2d(channels, channels, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels, channels, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
...
def train_CNN(model, epochs, train_loader, test_loader, checkpoint_dir = "checkpoint", starting_epoch = 0):
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    epoch_number = 0
    model_dir = checkpoint_dir + ".pth.tar"
    ...
    for epoch in range(starting_epoch, epochs):
        ...
        # Saves for each epoch
        checkpoint = {'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
        epoch_dir = checkpoint_dir + f"-{epoch + 1}E" + ".pth.tar"
        save_checkpoint(checkpoint, epoch_dir)

        checkpoint = {'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
        save_checkpoint(checkpoint, model_dir)
...
start_time = time.time()
train_CNN(vhse_model, epochs, train_loader, test_loader, "vhse_chkpt_300", 0)
print(f"Elapsed time: {time.time() - start_time}")
...
start_time = time.time()
train_CNN(pt_model, epochs, train_loader, test_loader, "pt_chkpt_300", 0)
print(f"Elapsed time: {time.time() - start_time}")
